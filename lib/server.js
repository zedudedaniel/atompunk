...
const page = {
    ...
    getHTML: function() {
        this.addBodyElement('<script>var knowledge;' +
            'var state;' +
            '$(document).ready(function () {' +
            this.plugins.join('.init(); ')+'.init();' +
            '});</script>'
        );
...
function setupEndpoints() {
    app.get('/', function (req, res) {
        //let page = new JSDOM('');
        //page.body = page.window.document.getElementsByTagName("body")[0];
        //page.head = page.window.document.getElementsByTagName("head")[0];
